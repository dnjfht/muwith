  ...
  return (
    <div className="flex">
      <div
        ...
        className="grid grid-rows-[1fr_6fr]"
      >
        <div
          className={`${
        ...
        >
          <Link
            href="/"
            className={`${
              pathname === "/" ? "" : "text-[#a6a6a6]"
            } flex items-center gap-x-3`}
          >
            {pathname === "/" ? <HiHome /> : <HiOutlineHome />}

            <p className={`${width > 250 ? "block" : "hidden"} text-[1rem]`}>
              홈
            </p>
          </Link>
        ...
        <div className="mt-2 p-[6px] box-border bg-[#ebebeb] rounded-lg shadow-lg text-[1.5rem]">
          <div
            className={`${width > 250 && "flex justify-between items-center"}`}
          >
            <button
              onClick={(e) => {
                e.preventDefault();
